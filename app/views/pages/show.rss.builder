...
xml.rss(:version=>"2.0"){
  xml.channel{
    ...
    @pages.each do |page|
      ...
      xml.item do
        xml.title(page.title)
        xml.description(page.main_content_column.blank? ? page.description : render_page_contents_for_rss(page.contents.active.find(:all, :conditions => ["column_position = ? AND (resource_type = ? OR resource_type = ?)", page.main_content_column, 'ContentTextfield', 'ContentImage'])))
